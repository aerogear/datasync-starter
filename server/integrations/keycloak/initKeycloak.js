...
const PUBLIC_CLIENT_NAME = "datasync-starter-client";
const BEARER_CLIENT_NAME = "datasync-starter-server";
let PUBLIC_CLIENT;
...
async function prepareKeycloak() {
  try {
    ...
    BEARER_CLIENT = clients.find(
      (client) => client.clientId === BEARER_CLIENT_NAME
    );
...
async function assignRealmRolesToUser(user, userIdUrl) {
  for (let roleToAssign of user.realmRoles) {
    ...
    const selectedRealmRole = realmRoles.find(
...
async function assignClientRolesToUser(user, client, clientRoles, userIdUrl) {
  for (let roleToAssign of user.clientRoles) {
    console.log(
      `assigning client role ${roleToAssign} from client ${client.clientId} on user ${user.name}`
    );
    const selectedClientRole = clientRoles.find(
      (clientRole) => clientRole.name === roleToAssign
    );
    if (selectedClientRole) {
      await assignClientRoleToUser(userIdUrl, client, selectedClientRole);
    } else {
      console.error(
        `client role ${roleToAssign} does not exist on client ${client.clientId}`
      );
    }
  }
}
